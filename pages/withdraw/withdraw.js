...
function getproinfo(pagesize, p, mythis) {
        ...
        wx.request({
                url: 'https://axure.xinice.com/index.php/index/index/withdrawcc',
                method: 'GET',
                data: {
                        pagesize: pagesize,
                        p: p,
                ...
                success: function (res) {
                        ...
                        // console.log(p);
                        // console.log(res.data.wcc.length);

                        //如果搜出来的结果=0，就说明后面已经没数据可加载了，所以将state设为0
                        if (res.data.wcc.length == 0){
                                ...
                                if (res.data.wcc.length < mythis.data.pagesize){
                                        var state1 = 0;
                                }      
                                //循环将结果集追加到数组后面
                                for (var i = 0; i < res.data.wcc.length; i++) {
                                        mythis.data.allProject.push(res.data.wcc[i]);
                                }
                                mythis.setData({
